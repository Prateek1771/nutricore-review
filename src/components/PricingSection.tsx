    ...
    <section ref={sectionRef} id="pricing" className="py-32 bg-nc-surface-deep">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          {/* Free */}
          <div className="pricing-card p-1.5 rounded-[calc(1.5rem+6px)] bg-black/[0.025] ring-1 ring-black/[0.04]">
            <div className="rounded-nc-lg bg-nc-surface p-7 h-full" style={{ boxShadow: 'var(--shadow-nc-card)' }}>
                ...
                {freeTier.map((item) => (
                  <li key={item.label} className="flex items-center gap-3">
                    <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${item.included ? 'bg-nc-mint' : 'bg-black/8'}`}>
                      {item.included
                        ? <Check size={10} weight="bold" className="text-nc-dark" />
                        : <X size={10} weight="bold" className="text-nc-text-muted" />
                      }
                    </span>
